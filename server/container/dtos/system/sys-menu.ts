...
export const CreateSysMenuDto = z.object({
  ...
  menuType: z.enum(MenuType.values, { invalid_type_error: '菜单类型参数格式不正确' }),
  ...
  isFrame: z.enum(YesOrNo.values, { invalid_type_error: '是否外链参数格式不正确' }).optional(),
  isCache: z.enum(YesOrNo.values, { invalid_type_error: '是否缓存参数格式不正确' }).optional(),
  isVisible: z.enum(YesOrNo.values, { invalid_type_error: '是否显示参数格式不正确' }).optional(),
